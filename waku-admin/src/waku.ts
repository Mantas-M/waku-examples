...
// import * as wakuCreate from '@waku/sdk/dist/create.js'
...
import {
  ChatMessagePayload,
  ChatPublicKeyMessagePayload,
  ForwardedMessagePayload,
  PublicKeyMessagePayload,
} from './types'

export class PublicKeyMessage {
  private payload: PublicKeyMessagePayload
  ...
  constructor(payload: PublicKeyMessagePayload) {
  ...
  static decode(bytes: Uint8Array) {
    const payload: PublicKeyMessagePayload = PublicKeyMessage.Type.decode(
      bytes,
...
export class ChatPublicKeyMessage {
  private payload: ChatPublicKeyMessagePayload
  ...
  constructor(payload: ChatPublicKeyMessagePayload) {
    this.payload = payload
  }
  ...
  static decode(bytes: Uint8Array) {
    const payload = ChatPublicKeyMessage.Type.decode(
      bytes,
    ) as unknown as ChatPublicKeyMessagePayload
...
export class ForwardedMessage {
  private payload: ForwardedMessagePayload
  ...
  constructor(payload: ForwardedMessagePayload) {
    this.payload = payload
  }
  ...
  static decode(bytes: Uint8Array) {
    const payload = ForwardedMessage.Type.decode(
      bytes,
    ) as unknown as ForwardedMessagePayload
...
export function getTripChatContentTopic(tripId: number) {
  return `/rentality/1/pm-trip-${tripId}/proto`
}
export function getUserKeysContentTopic(address: string) {
...
export class Waku {
  ...
  static async listenForJoinChatRequests(node /*: LightNode*/) {
